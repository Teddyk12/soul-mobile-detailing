...
export interface Booking {
  id: string;
  customerName: string; // Changed from 'name' for clarity
  ...
  date: string;
  time: string;
  notes: string; // Customer's initial notes
  adminNotes?: BookingNote[]; // Admin notes added over time
  createdAt: string;
  status: 'pending' | 'confirmed' | 'completed' | 'cancelled';
  previousBookings?: number; // Count of previous bookings by this customer
...
// ============================================
// CUSTOMER HISTORY & ADMIN NOTES
// ============================================

export function getCustomerHistory(emailOrPhone: string): Booking[] {
  const bookings = loadBookings();
  return bookings.filter(b =>
    b.email.toLowerCase() === emailOrPhone.toLowerCase() ||
    b.phone === emailOrPhone
  ).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}

export function addAdminNote(bookingId: string, adminUser: string, note: string): void {
  if (typeof window === 'undefined') return;
  ...
  if (booking) {
    ...
    booking.adminNotes.push(newNote);

    localStorage.setItem('bookings', JSON.stringify(bookings));
  }
}

export function enrichBookingWithHistory(booking: Booking): Booking {
  const history = getCustomerHistory(booking.email || booking.phone);
  const previousBookings = history.filter(b => b.id !== booking.id);

  return {
    ...booking,
    previousBookings: previousBookings.length,
    totalSpent: previousBookings
      .filter(b => b.status === 'completed')
      .reduce((sum, b) => {
        // Extract price from service string if it exists
        const priceMatch = b.service.match(/\$(\d+(?:\.\d{2})?)/);
        return sum + (priceMatch ? parseFloat(priceMatch[1]) : 0);
      }, 0)
  };
}

export function getBookingWithHistory(id: string): Booking | null {
  const bookings = loadBookings();
  const booking = bookings.find(b => b.id === id);
  return booking ? enrichBookingWithHistory(booking) : null;
}
...
export const getAvailability = loadAvailability;
export const updateBooking = (id: string, updates: Partial<Booking>) => {
  if (updates.status) {
    updateBookingStatus(id, updates.status);
  }
};
export const addAvailability = (slot: { id: string; date: string; time: string; available: boolean; bookingId: string | null }) => {
  addAvailabilitySlot(slot.date, slot.time);
};
export const deleteAvailability = deleteAvailabilitySlot;
